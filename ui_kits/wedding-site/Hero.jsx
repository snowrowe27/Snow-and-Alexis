...
function Hero({go}){
 return <div id="top" style={{position:'relative',background:'var(--cream-100)',padding:'var(--space-8) var(--gutter-page) var(--space-7)',overflow:'hidden',textAlign:'center'}}>
  <style>{"@keyframes heroChandSway{0%,100%{transform:rotate(-1.4deg)}50%{transform:rotate(1.4deg)}}@media (prefers-reduced-motion:reduce){.heroChand{animation:none!important}}"}</style>
  <span aria-hidden="true" className="heroChand" style={{position:'absolute',top:-2,left:'5%',width:118,height:143,display:'block',transformOrigin:'top center',animation:'heroChandSway 7s ease-in-out infinite',animationDelay:'-3.5s',background:'var(--villa-green)',WebkitMaskImage:'url('+ILL+'/chandelier.png)',maskImage:'url('+ILL+'/chandelier.png)',WebkitMaskRepeat:'no-repeat',maskRepeat:'no-repeat',WebkitMaskPosition:'top center',maskPosition:'top center',WebkitMaskSize:'contain',maskSize:'contain'}}/>
  <span aria-hidden="true" className="heroChand" style={{position:'absolute',top:-2,right:'5%',width:118,height:143,display:'block',transformOrigin:'top center',animation:'heroChandSway 7s ease-in-out infinite',background:'var(--villa-green)',WebkitMaskImage:'url('+ILL+'/chandelier.png)',maskImage:'url('+ILL+'/chandelier.png)',WebkitMaskRepeat:'no-repeat',maskRepeat:'no-repeat',WebkitMaskPosition:'top center',maskPosition:'top center',WebkitMaskSize:'contain',maskSize:'contain'}}/>
  <div style={{maxWidth:'var(--container-text)',margin:'0 auto'}}>
   ...
   <h1 style={{fontFamily:'var(--font-display)',fontSize:'clamp(2.75rem,7vw,5.75rem)',lineHeight:'var(--leading-tight)',margin:'10px 0 0',color:'var(--villa-green)'}}>ALEXIS <span style={{color:'var(--miami-orange)'}}>&amp;</span> MATTHEW</h1>
   <div style={{fontFamily:'var(--font-hand)',fontWeight:600,fontSize:'clamp(1.875rem,3vw,2.375rem)',color:'var(--orange-600)',marginTop:8}}>or Lexi and Snow, if you know us</div>
  </div>
